#  - assign kafka cluster and topic then send event
#  - crawl one stock info one sec one time
...
def fetch_price(producer, symbol):
	...
	price = tsla.get_price()
    	...
    	producer.send(topic=topic_name, value=bytes(price), timestamp_ms=time.time())
